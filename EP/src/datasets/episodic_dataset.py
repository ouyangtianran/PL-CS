...
class EpisodicDataset(Dataset):
    def __init__(self, labels, sampler, size, transforms):
        ...
        self.label_ind = np.ones(self.labelset.max()+1,dtype=int)*-1
        for i,v in enumerate(self.labelset):
            self.label_ind[v] = i
    ...
    def gen_few_shot_task(self, nclasses, size):
        ...
        # 使用auglabel，需增加一层索引转换self.label_ind
        classes = np.random.choice(self.labelset, nclasses, replace=False)
        starts = self.starts[self.label_ind[classes]]
        reminders = self.lengths[self.label_ind[classes]] - starts
        if np.min(reminders) < size:
            return None
        sample_indices = np.array(
            [self.clss_idx[self.label_ind[classes[i]]][starts[i]:(starts[i] + size)] for i in range(len(classes))])
        sample_indices = np.reshape(sample_indices, [nclasses, size]).transpose()
        self.starts[self.label_ind[classes]] += size
        return sample_indices.flatten()
