...
class RotatedNonEpisodicTieredimagenet(Dataset):
    ...
    def __init__(self, data_root, split, transforms, rotation_labels=[0, 1, 2, 3], filename = None, aug_label=None, **kwargs):
        """ Constructor
        ...
        Args:
            ...
            disjoint: whether to create disjoint splits.
        """
        self.data_root = os.path.join(data_root, filename)
        data = np.load(self.data_root % self.split_paths[split])
        self.features = data["X"]
        if aug_label is None:
            self.labels = data["cluster_label"]
        else:
            self.labels = aug_label
        # self.cluster_centers = data['cluster_centers']
        self.transforms = transforms
