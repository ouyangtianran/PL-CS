...
class FinetuneWrapper(BaseWrapper):
    ...
    def __init__(self, model, nclasses, exp_dict, select_entropy=False, data = None, center = None, aug_label = None, checkpoint = None, device=None):
        """ Constructor
        Args:
        ...
        if center is not None:
            c_list, c_centers, ind_sorted = self.load_clusters_1(data, center, aug_label) # 只用于补点，应该不用auglabel
        ...
        if checkpoint is not None:
            if self.exp_dict["model"]["name"] == 'finetuning_un':
                pkl_path = os.path.join(checkpoint, 'score_list_best.pkl')
                accuracy = haven.load_pkl(pkl_path)[-1]["val_accuracy"]
                try:
                    self.model.load_state_dict(torch.load(os.path.join(checkpoint, 'checkpoint_best.pth'))['model'], strict=False)
                    if accuracy > best_accuracy:
                        best_path = os.path.join(checkpoint, 'checkpoint_best.pth')
                        best_accuracy = accuracy
                except:
                    print("error in load checkpoint")
            else:
                base_path = os.path.join(self.exp_dict['pretrained_weights_root'], checkpoint)
                exp_dict_path = os.path.join(base_path, 'exp_dict.json')
                if not os.path.exists(exp_dict_path):
                        print(base_path + ":not exist.")
    ...
    def load_clusters_1(self, data, center, aug_label = None):
        c_centers = center["cluster_centers"]
        if self.device is not None:
            c = torch.tensor(c_centers).cuda(device=self.device, non_blocking=True)
        else:
            c = torch.tensor(c_centers).cuda(non_blocking=True)
        dist = pairwise_distances(c, c, 'l2')
        _, ind_sorted = torch.sort(dist, dim=1)
        ...
        if aug_label is None:
            ...
            Y = aug_label
            Y_u, counts = np.unique(Y, return_counts=True)
            n_min = 25
            for y, count in zip(Y_u,counts):
                if count < n_min:
                    n_repeat = np.ceil(n_min / count)
                    ind = Y == y  # 在全部点中定位属于此簇的点
                    x0 = np.repeat(X[ind], n_repeat, axis=0)
                    # data_list.append(x0)
                    data_dict[y]=x0
                else:
                    # data_list.append(X[Y == y])
                ...
                label_names.append(y)
        if len(data_dict) > 0:
            c_list = data_dict  # 类别缩减后序号和类别号对不上了，改用字典
        else:
            c_list = data_list
    ...
    def train_on_batch(self, batch, pem_eta = 0.9):
    ...
    def train_on_loader(self, data_loader,pem_eta=0.9, max_iter=None, debug_plot_path=None):
        ...
        for batch_idx, batch in enumerate(data_loader):
            loss = self.train_on_batch(batch, pem_eta) / self.exp_dict["tasks_per_batch"]
