...
__all__ = ['ModelAgnosticMetaLearningAug', 'MAML', 'FOMAML']


class ModelAgnosticMetaLearningAug(object):
    ...
    def __init__(self, model, optimizer=None, step_size=0.1, first_order=False,
                 ...
                 dataloader=None, clu_aug_label_matrix = None):
        ...
        # load clusters
        c_list, c_centers, ind_sorted = load_clusters(dataloader,self.device)
        self.c_list, self.c_centers, self.ind_sorted = c_list, c_centers, ind_sorted
        self.clu_aug_label_matrix = clu_aug_label_matrix
    ...
    def get_outer_loss(self, batch, changed_num = 0):
    ...
    def train(self, dataloader, log_file, max_batches=500, verbose=True, pem_eta=0.9, **kwargs):
        changed_num = 0
        with tqdm(total=max_batches, disable=not verbose, **kwargs) as pbar:
            for results in self.train_iter(dataloader, max_batches=max_batches, 
                ...
                changed_num += results['changed_num']
        print("changed_num:"+str(changed_num))
        with open(log_file, 'a') as f:
            f.write("changed_num:"+str(changed_num)+"\n")
    ...
    def train_iter(self, dataloader, max_batches=500, pem_eta=0.9):
        ...
        while num_batches < max_batches:
            for batch in dataloader:
                ...
                outer_loss, results = self.get_outer_loss(batch, changed_num)
                yield results
...
MAML = ModelAgnosticMetaLearningAug

class FOMAML(ModelAgnosticMetaLearningAug):
